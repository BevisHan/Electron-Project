...
function configFileService(fileConfig) {
    ...
    ipcMain.handle('download-report-file', (evt, options) => {
        var downloadOptions = {
            id: options.id,
            url: options.url,
            name: options.name,
            isAutoOpen: options.isAutoOpen,
            localPath: path.join(options.targetSource.specifiedDirectory, options.name),
            path: options.targetSource.specifiedDirectory
        };
        return download.downloadFile(downloadOptions);
    });
